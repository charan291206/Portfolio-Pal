...
const dataFile = path.join(__dirname, 'projects-data.json');
...
function saveProjects(projectsToSave) {
  try {
    fs.writeFileSync(dataFile, JSON.stringify(projectsToSave, null, 2), 'utf8');
  } catch (error) {
...
// ============================================
// IN-MEMORY PROJECT STORAGE (Persisted to JSON file)
// ============================================
...
// Delete project endpoint (students can delete their own projects)
app.delete('/api/project/:id', authMiddleware, (req, res) => {
  ...
  if (req.session.user.username !== project.studentUsername && req.session.user.role !== 'ADMIN') {
